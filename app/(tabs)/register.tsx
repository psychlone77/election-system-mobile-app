...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    console.log("Attempting register with NIC:", NIC);
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Store NIC for future logins
        if (Platform.OS === "web") {
          localStorage.setItem("userNIC", NIC);
        } else {
          await SecureStore.setItemAsync("userNIC", NIC, {
            keychainAccessible: SecureStore.WHEN_UNLOCKED_THIS_DEVICE_ONLY,
          });
        }

        showSuccessToast("Registration Successful", response.data.message || `Welcome ${NIC}!`);
        ...
        showErrorToast(
          "Registration Failed",
          response.data.message || "Invalid credentials. Try again."
        );
      }
    } catch (err) {
